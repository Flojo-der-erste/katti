...
from DataFeeds import import_loader

#celery -A KattiApp beat -S CeleryBeatMongo.schedulers.MongoScheduler
  ...
  #          host = app.conf.get('mongodb_scheduler_url')
   #     elif hasattr(app.conf, "CELERY_MONGODB_SCHEDULER_URL"):
    ...
    def get_from_database(self):
        self.sync()
        d = {}
        for doc in PeriodicTask.objects.filter(enabled=True):
