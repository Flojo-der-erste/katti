...
class Interval(EmbeddedDocument):
    ...
    @classmethod
    def build_from_json(cls, interval_json):
        new_interval = Interval(every=interval_json.get('every', 1),
                                period=interval_json.get('period', 'seconds'))
        return new_interval
...
class Crontab(EmbeddedDocument):
        ...
        meta = {'collection': 'feed_entries', 'abstract': True}
        ...
        @classmethod
        def build_from_json(cls, **crontab_json):
            new_crontab = Crontab(minute=crontab_json.get('minute', '*'),
                                  hour=crontab_json.get('hour', '*'),
                                  day_of_week=crontab_json.get('day_of_week', '*'),
                                  day_of_month=crontab_json.get('day_of_month', '*'),
                                  month_of_year=crontab_json.get('month_of_year', '*'))
            return new_crontab
...
class PeriodicTask(DynamicDocument):
    ...
    run_immediately = BooleanField(default=True)
    no_changes = False
