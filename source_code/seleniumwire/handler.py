...
class InterceptRequestHandler:
    ...
    def request(self, flow):
        ...
        if flow.request.headers.get('tardis_header'):
            header_value = flow.request.headers.pop('tardis_header')
            request_id, tab_id, parent_frame_id, frame_id, initiator, type, timestamp = header_value.split(' ')
        else:
            request_id, tab_id, parent_frame_id, frame_id, initiator, type, timestamp = None, None, None, None, None, None, None

            # Convert to one of our requests for handling
        request = self._create_request(flow)
        try:
            request.browser_request_id = request_id
            request.browser_tab_id = tab_id
            request.frame_id = frame_id
            request.parent_frame_id = parent_frame_id
            request.initiator = initiator
            request.type = type
            request.timestamp = timestamp
            ip, port = flow.server_conn.ip_address
